...
mongo_csv_paths = [
    "C:\\Users\\lucac\\PycharmProjects\\pythonProject\\MongoDB\\tempi_di_risposta_prima_esecuzione_MDB.csv",
    "C:\\Users\\lucac\\PycharmProjects\\pythonProject\\MongoDB\\tempi_di_risposta_media_30_MDB.csv",
]
...
for query in queries:
    ...
    for size in dataset_sizes:
        values_mongo_prima_esecuzione = data_mongo_query_prima_esecuzione[data_mongo_query_prima_esecuzione['Dataset'] == size]['Millisecondi']
        values_neo4j_prima_esecuzione = data_neo4j_query_prima_esecuzione[data_neo4j_query_prima_esecuzione['Dataset'] == size]['Millisecondi']

        plt.bar([f"{size} (MongoDB)", f"{size} (Neo4j)"], [values_mongo_prima_esecuzione.values[0], values_neo4j_prima_esecuzione.values[0]],
    ...
    plt.figure(figsize=(10, 6))
    for size in dataset_sizes:
        values_mongo_media_30 = data_mongo_query_media_30[data_mongo_query_media_30['Dataset'] == size]['Media']
        values_neo4j_media_30 = data_neo4j_query_media_30[data_neo4j_query_media_30['Dataset'] == size]['Media']

        # Estrae intervalli di confidenza
        confidence_intervals_mongo = data_mongo_query_media_30[data_mongo_query_media_30['Dataset'] == size][
            'Intervallo di Confidenza (Min, Max)']
        confidence_intervals_neo4j = data_neo4j_query_media_30[data_neo4j_query_media_30['Dataset'] == size][
            'Intervallo di Confidenza (Min, Max)']
        ...
        mongo_yerr = [[values_mongo_media_30.values[0] - conf_mongo_min[0]], [conf_mongo_max[0] - values_mongo_media_30.values[0]]]
        neo4j_yerr = [[values_neo4j_media_30.values[0] - conf_neo4j_min[0]], [conf_neo4j_max[0] - values_neo4j_media_30.values[0]]]

        # Rappresenta l'intervallo di confidenza per MongoDB e Neo4j
        plt.bar(f"{size} (MongoDB)", values_mongo_media_30.values[0], yerr=mongo_yerr, capsize=5, color=color_mongo, label='MongoDB')
        plt.bar(f"{size} (Neo4j)", values_neo4j_media_30.values[0], yerr=neo4j_yerr, capsize=5, color=color_neo4j, label='Neo4j')
